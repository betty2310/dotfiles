...
# Dunst Time
send_notification() {
	if command >/dev/null 2>&1 -v notify-send; then
		if [[ "${store}" == "save" ]]; then
			#dunstify -i ~/.config/dunst/icons/screenshot.svg 'Screenshot saved!'
			dunstify -h string:fgcolor:#a3be8c '      ' -a "Screenshots" -i ~/.config/dunst/icons/icons8-screenshot.svg "Save to Pictures!!!!"
		elif [[ "${store}" == "clip" ]]; then
			#dunstify -h string:fgcolor:#a3be8c '      ' -i ~/.config/dunst/icons/clipboard.svg 'Screenshot passed to clipboard!'
			dunstify -h string:fgcolor:#ebcb8b '      ' -a "Screenshots" -i ~/.config/dunst/icons/icons8-copy-to-clipboard.svg "Passed to clipboard!!!!"
		fi
	fi
}
