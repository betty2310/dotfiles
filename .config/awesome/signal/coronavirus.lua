...
local temp_file = "/tmp/awesomewm-signal-coronavirus-" .. country
...
helpers.remote_watch(coronavirus_script, update_interval, temp_file, function(stdout)
    ...
    if cases_total then
        awesome.emit_signal("signal::coronavirus", cases_total, cases_today, deaths_total, deaths_today)
    else
        -- Remove temp_file to force an update the next time
        awful.spawn.with_shell("rm " .. temp_file)
        awesome.emit_signal("signal::coronavirus", -1, -1, -1, -1)
    end
end)
