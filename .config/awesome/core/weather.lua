...
local temp_file = "/tmp/awesomewm-core-weather-"..city_id.."-"..units
...
helpers.remote_watch(weather_details_script, update_interval, temp_file, function(stdout)
    ...
    if icon_code == "..." then
        ...
        awesome.emit_signal("core::weather", 999, "Weather unavailable", "")
    else
        awesome.emit_signal("core::weather", tonumber(temperature), description, icon_code)
    end
end)
