...
local properties = { "50", "50", "x.color1", "x.color2", "x.color3", "seconds", "true" }
...
function awmodoro:begin()
    if not data[self].timer.started then
        if data[self].begin_callback then
            data[self].begin_callback()
        end
        data[self].elapsed = 0
...
function awmodoro:finish()
    ...
    if data[self].finish_callback then
        data[self].finish_callback()
    end
end
